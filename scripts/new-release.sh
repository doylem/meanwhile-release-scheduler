...
configure_label() {
  case "$1" in
    ...
    MW)
      ...
      PSB_ARTIST="MW - Artist name.psb";  LAYER_ARTIST="ARTIST_NAME"
      PSB_TRACKS="MW - track names.psb";  LAYER_TRACKS="TRACK NAMES"
      PSB_CAT="MW - Logo and cat.psb";    LAYER_CAT="RELEASE_CATALOGUE"
      SMART_OBJECTS_JS='[
...
var LAYER_ARTIST = "$(esc "$LAYER_ARTIST")";
var LAYER_TRACKS = "$(esc "$LAYER_TRACKS")";
var LAYER_CAT    = "$(esc "$LAYER_CAT")";
var SMART_OBJECTS = ${SMART_OBJECTS_JS};

function main() {
  updatePSBText(ASSETS_DIR + "/" + PSB_ARTIST, LAYER_ARTIST, "$(esc "$ARTIST_NAME")");
  updatePSBText(ASSETS_DIR + "/" + PSB_TRACKS, LAYER_TRACKS, "${TRACK_TEXT}");
  updatePSBText(ASSETS_DIR + "/" + PSB_CAT,    LAYER_CAT,    CAT_NUMBER);

  var tif = open(new File(ASSETS_DIR + "/" + TIF_NAME));
  relinkSmartObjects(tif);
  tif.save();
  tif.close(SaveOptions.DONOTSAVECHANGES);

  alert("Done!\n\n" + CAT_NUMBER + " is ready.\nOpen your mark PSB from:\n" + ASSETS_DIR);
}

function updatePSBText(psbPath, layerName, newText) {
...
function relinkSmartObjects(doc) {
  for (var i = 0; i < SMART_OBJECTS.length; i++) {
    var matches = findAllLayers(doc.layers, SMART_OBJECTS[i].layerName);
    if (matches.length === 0) alert("No layer named '" + SMART_OBJECTS[i].layerName + "' found. Check SMART_OBJECTS in new-release.sh.");
    for (var j = 0; j < matches.length; j++) relinkLayer(doc, matches[j], ASSETS_DIR + "/" + SMART_OBJECTS[i].psbFile);
  }
}

function relinkLayer(doc, layer, filePath) {
  try {
    doc.activeLayer = layer;
    var d = new ActionDescriptor();
    d.putPath(charIDToTypeID("null"), new File(filePath));
    executeAction(stringIDToTypeID("relinkToFile"), d, DialogModes.NO);
  } catch (e) { alert("Relink failed for '" + layer.name + "':\n" + e.message); }
}
...
function findAllLayers(layers, name) {
  var out = [];
  for (var i = 0; i < layers.length; i++) {
    if (layers[i].name === name) out.push(layers[i]);
    if (layers[i].typename === "LayerSet") { var n = findAllLayers(layers[i].layers, name); for (var j = 0; j < n.length; j++) out.push(n[j]); }
  }
  return out;
}
...
# ── OPEN IN PHOTOSHOP ─────────────────────────────────────────────────────────
print ""
print -P "  %F{cyan}Opening Photoshop...%f"
open -a "Adobe Photoshop 2026" "$JSX_FILE"
print -P "  %F{white}Photoshop is running. Watch for an alert when it finishes.%f"
print ""
print -P "  %F{white}Then open your mark file from:%f"
print -P "  %F{cyan}${ASSETS_DIR}/%F{white}${MARK_PSB}%f"
print -P "  %F{white}Add your artwork and save -- the TIF updates automatically.%f"
print ""
